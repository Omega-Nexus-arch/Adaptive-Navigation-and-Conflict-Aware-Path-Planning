...
def render_landmarks(spec: WorldSpec) -> str:
    """Emit the ramp and deck landmarks as a flat, trivially parseable table.

    WHY THIS FILE EXISTS.

    The C++ slope tests need to know where the ramps are and how steep they
    are. They used to carry that as hardcoded constants -- probe points at
    (7.0, 5.85) and gradients of 8.7 and 14.8 degrees. The world is generated,
    it was regenerated many times, and those constants were never updated, so
    five tests had been failing silently against a warehouse that no longer
    existed. The same drift hit the README and the traceability table.

    Deriving beats duplicating. This is written by the same pass that writes
    the world, so a ramp cannot move without the tests following it.

    Format is one record per line, whitespace separated, so the C++ side needs
    an ``ifstream`` and nothing else:

        ramp <name> <probe_x> <probe_y> <degrees>
        deck <name> <centre_x> <centre_y> <height>
    """
    lines = [
        '# Generated by world_builder.render_landmarks -- do not edit.',
        '# ramp <name> <probe_x> <probe_y> <degrees>',
        '# deck <name> <centre_x> <centre_y> <height>',
    ]
    for ramp in spec.ramps:
        middle = (ramp.start + ramp.end) / 2.0
        if ramp.axis == 'x':
            probe_x, probe_y = middle, ramp.cross
        else:
            probe_x, probe_y = ramp.cross, middle
        lines.append(
            f'ramp {ramp.name} {probe_x:.4f} {probe_y:.4f} {ramp.slope_deg:.4f}')
    for deck in spec.decks:
        lines.append(
            f'deck {deck.name} {(deck.x_min + deck.x_max) / 2.0:.4f} '
            f'{(deck.y_min + deck.y_max) / 2.0:.4f} {deck.height:.4f}')
    return '\n'.join(lines) + '\n'
...
def write_all(spec: WorldSpec, package_dir: str, resolution: float = 0.05) -> Dict[str, str]:
    ...
    landmarks_path = os.path.join(maps_dir, 'warehouse_landmarks.txt')
    with open(landmarks_path, 'w', encoding='utf-8') as handle:
        handle.write(render_landmarks(spec))
